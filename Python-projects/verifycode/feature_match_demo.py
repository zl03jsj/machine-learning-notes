...
im = cv2.imread('img/origin/009.bmp')
gray = cv2.cvtColor(im, code=cv2.COLOR_BGR2GRAY)
...
out_image = im.copy()
for k in kp:
    cv2.drawKeypoints(gray, kp, out_image, color=255, flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
    # cv2.circle(im,(int(k.pt[0]),int(k.pt[1])),1,(0,255,0),-1)
cv2.imshow('SURF_features', out_image)
cv2.waitKey()
cv2.destroyAllWindows()
